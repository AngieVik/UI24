...
* **Ancho Colapsado (`--col-w`):** `40px`. (Mostrando exclusivamente el rail de iconos para maximizar espacio).
* **Ancho Expandido (`--col-w-expanded`):** `160px`. (Mostrando iconos, etiquetas truncadas y chevrons).
...
* **Selección de Hoja (Auto-colapso):** Al hacer clic en una ruta terminal (hoja funcional, ej. *Doc-10*), el contenido renderiza en el `home_area`. Si la columna estaba expandida a 160px, se **auto-contrae** a 40px para despejar el área de trabajo táctil.
...
* **Toggle Expand/Collapse:** Botón anclado rígidamente en la base absoluta de la columna. Posición inmutable. Alterna 40px <-> 160px.
...
* **Rail Icono:** Contenedor de `w-[40px]` exacto (alineado al estado colapsado para evitar temblores).
