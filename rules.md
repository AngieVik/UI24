...
* **Capitalización (Sentence case estricto):** Se han revisado absolutamente todos los textos de la interfaz. Ahora todo emplea el formato "Primera letra mayúscula, resto minúscula" (ej. "Paleta semántica", "En ruta", "Control operativo U24"). Las MAYÚSCULAS SOSTENIDAS están estrictamente restringidas a acrónimos (ej. DRP, PSA, ITV, UTC) y a botones de acción primaria destructiva como por ejemplo "ELIMINAR REGISTRO".
...
## 2. Sistema de diseño, colores y componentes (UI Kit)

La interfaz no tendrá secciones de diferentes estilos. La uniformidad visual es inquebrantable.

* **Librería de componentes base:** Se usarán de forma exclusiva todos los componentes de `ui.shadcn.com`. Queda prohibido construir componentes interactivos complejos (modales, selectores, tooltips) desde cero si existe un equivalente en esta librería.
...
* **Regla minimalistal:** Minimizar los espaciados (`gap`), márgenes y paddings. Preferible una fila compacta con `text-xs p-1` a una columna con botones desproporcionados.
...
* **Políticas RLS de prohibición incondicional:** Para tablas con inmutabilidad de negocio (ej. `doc1_asistencias`), las políticas RLS de UPDATE y DELETE deben devolver `USING (FALSE)` de forma estricta e incondicional — independientemente de los claims del JWT o del rol del usuario. Esta tabla debe aislarse de las políticas genéricas aplicadas a otros documentos de la misma familia. Ver `rbac_y_permisos.md §5` y `logic.md §13.3`.
