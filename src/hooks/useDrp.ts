...
export function useDrp() {
  ...
  const cargarDrps = useCallback(async () => {
    ...
    try {
      const { data, error: err } = await supabase
        .from('drps')
        .select('id_drp, estado, id_coordinacion, timestamp_preparacion, timestamp_inicio, timestamp_fin, timestamp_cancelacion')
  ...
  const cargarDetalle = useCallback(async (idDrp: string) => {
    const [dotRes, persRes, descRes] = await Promise.all([
      ...
      supabase
        ...
        .select('id_descuadre, id_item, location_origen, location_destino, cantidad_diferencia, estado')
  ...
  const crearDrp = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      const { data, error: err } = await supabase
        .rpc('rpc_crear_drp', { p_mutation_uuid: mutationUuid })
      if (err) throw err
      await cargarDrps()
  ...
  const transicionarDrp = useCallback(async (idDrp: string, accion: 'preparar' | 'iniciar' | 'finalizar' | 'archivar'): Promise<boolean> => {
    if (!isOnline) {
      setError('El módulo DRP requiere conexión a red.')
      return false
    }
    try {
      const { error: err } = await supabase
        .rpc('rpc_transicionar_drp', { p_id_drp: idDrp, p_accion: accion })
      if (err) throw err
      await cargarDrps()
      return true
    } catch (e) {
  ...
  const cancelarDrp = useCallback(async (idDrp: string, motivo?: string): Promise<boolean> => {
    if (!isOnline) {
      setError('El módulo DRP requiere conexión a red.')
      return false
    }
    try {
      const { error: err } = await supabase
        .rpc('rpc_cancelar_drp', { p_id_drp: idDrp, p_motivo: motivo || undefined })
      if (err) throw err
      await cargarDrps()
      return true
    } catch (e) {
  ...
  const agregarDotacion = useCallback(async (idDrp: string, matricula: string): Promise<boolean> => {
    if (!isOnline) {
      setError('El módulo DRP requiere conexión a red.')
      return false
    }
  ...
  const agregarPersonal = useCallback(async (idDrp: string, idNombre: string, zona?: string): Promise<boolean> => {
    if (!isOnline) {
      setError('El módulo DRP requiere conexión a red.')
      return false
    }
    try {
      const { error: err } = await supabase
        .rpc('rpc_agregar_personal_pie_drp', {
          p_mutation_uuid: crypto.randomUUID(),
          p_id_drp: idDrp,
  ...
  const asignarMochila = useCallback(async (idDrp: string, idMochila: string): Promise<boolean> => {
    if (!isOnline) {
      setError('El módulo DRP requiere conexión a red.')
      return false
    }
    try {
      const { error: err } = await supabase
        .rpc('rpc_asignar_mochila_a_drp', { p_id_mochila: idMochila, p_id_drp: idDrp })
      if (err) throw err
      await cargarDetalle(idDrp)
      return true
    } catch (e) {
  ...
  const resolverDescuadre = useCallback(async (idDescuadre: string, resolucion: 'Resuelto' | 'Archivado', notas?: string): Promise<boolean> => {
    if (!isOnline) {
