    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-2 pb-2">
        ...
        <div className="flex items-center gap-1">
          {data && (
            <Badge variant={estadoBadgeVariant(data.estado)}>{ESTADO_LABEL[data.estado]}</Badge>
          )}
      ...
      <CardContent>
        ...
        {!isLoading && isError && (
          <p className="text-sm text-destructive">No se pudo cargar el DRP activo. Reintentando…</p>
        )}
        ...
        {!isLoading && !isError && data && (
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs font-light text-muted-foreground">
            <span>
              Coord.: <span className="font-medium text-foreground">{data.id_coordinacion}</span>
            </span>
            <span>
              Inicio:{' '}
              <span className="font-medium text-foreground">
                {formatFecha(data.timestamp_inicio)}
              </span>
            </span>
            <span>
              Preparación:{' '}
              <span className="font-medium text-foreground">
                {formatFecha(data.timestamp_preparacion)}
              </span>
            </span>
