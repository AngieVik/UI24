...
export function ResumenDrpScreen() {
    ...
    return (
      <div className="mx-auto flex max-w-xl flex-col items-center gap-3 px-6 py-16 text-center">
        ...
        <p className="font-body text-sm text-muted-foreground">
          El resumen DRP requiere conexión en tiempo real.
        </p>
      </div>
    )
  }
  ...
  const finales = drps.filter(
    (d) =>
      d.estado === 'Finalizado' || d.estado === 'Finalizado_Retenido' || d.estado === 'Cancelado'
  )
      ...
      {!loading && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            {
              label: 'Descuadres',
              value: descuadresPendientes.length,
              variant: descuadresPendientes.length > 0 ? ('warn' as const) : ('secondary' as const),
            },
          ].map((m) => (
            <Card key={m.label}>
              ...
              <TableBody>
                {drps.map((d) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Badge variant={ESTADO_VARIANT[d.estado] ?? 'info'}>{d.estado}</Badge>
                    </TableCell>
