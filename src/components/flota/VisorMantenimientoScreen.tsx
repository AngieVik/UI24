...
export function VisorMantenimientoScreen({ vista }: { vista?: 'tabla' | 'badges' | 'filtros' | 'detalle' }) {
  ...
  const criticos = vehiculos.filter((v) =>
    v.condicion_tecnica === 'fuera_de_servicio' || v.condicion_tecnica === 'averia_grave'
  )

  const revisar = vehiculos.filter((v) =>
    v.condicion_tecnica === 'revision_pendiente' || v.condicion_tecnica === 'averia_leve'
  )
  ...
  return (
    <div className="mx-auto flex w-full max-w-screen-xl flex-col gap-3 p-3">
      ...
      <Tabs value={tab} onValueChange={(t) => { setTab(t); if (t !== 'detalle') setDetalleId(null) }}>
        <TabsList className="w-full sm:w-auto">
          <TabsTrigger value="tabla"><Gauge className="size-3.5 mr-1" />Tabla</TabsTrigger>
          <TabsTrigger value="badges"><Tag className="size-3.5 mr-1" />Badges</TabsTrigger>
          <TabsTrigger value="filtros"><Filter className="size-3.5 mr-1" />Filtros</TabsTrigger>
          <TabsTrigger value="detalle"><Eye className="size-3.5 mr-1" />Detalle</TabsTrigger>
        </TabsList>
                  ...
                  <TableBody>
                    {vehiculos.map((v) => (
                      <TableRow key={v.matricula}>
                        ...
                        <TableCell>
                          <Badge variant={CONDICION_VARIANT[v.condicion_tecnica] ?? 'info'} className="text-xs">
                        ...
                        <TableCell>
                          <Button
                            ...
                            onClick={() => { setDetalleId(v.matricula); setTab('detalle') }}
            ...
            <div className="space-y-4">
              {criticos.length > 0 && (
                <Card className="border-destructive/50">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm text-destructive">Críticos — fuera de servicio</CardTitle>
                  </CardHeader>
                  <CardContent className="flex flex-wrap gap-2">
                    {criticos.map((v) => (
                      <Badge key={v.matricula} variant="destructive">{v.matricula}</Badge>
                    ))}
                  </CardContent>
                </Card>
              )}
              {revisar.length > 0 && (
                <Card className="border-yellow-500/50">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm text-yellow-700 dark:text-yellow-400">Revisión pendiente</CardTitle>
                  </CardHeader>
                  <CardContent className="flex flex-wrap gap-2">
                    {revisar.map((v) => (
                      <Badge key={v.matricula} variant="warn">{v.matricula}</Badge>
                    ))}
                  </CardContent>
                </Card>
              )}
              <Card>
                ...
                <CardContent className="flex flex-wrap gap-2">
                  {vehiculos.filter((v) => v.condicion_tecnica === 'operativo').map((v) => (
                    <Badge key={v.matricula} variant="ok">{v.matricula}</Badge>
                  ))}
                </CardContent>
              </Card>
            </div>
        ...
        <TabsContent value="filtros" className="mt-3">
          <div className="space-y-3">
            ...
            />
            {isLoading ? <Skeleton className="h-40 w-full" /> : filtrados.length === 0 ? (
              <Card>
              ...
              <div className="space-y-2">
                {filtrados.map((v) => (
                  <Card key={v.matricula} className="cursor-pointer hover:bg-muted/40" onClick={() => { setDetalleId(v.matricula); setTab('detalle') }}>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                  ...
                  <div>
                    <p className="text-xs font-bold uppercase text-muted-foreground">Condición técnica</p>
                    <Badge variant={CONDICION_VARIANT[detalleVehiculo.condicion_tecnica] ?? 'info'}>
                      {detalleVehiculo.condicion_tecnica.replace(/_/g, ' ')}
                    </Badge>
                  </div>
                  <div>
                    <p className="text-xs font-bold uppercase text-muted-foreground">Estado operativo</p>
                    <Badge variant={ESTADO_VARIANT[detalleVehiculo.estado_operativo] ?? 'info'}>
                      {detalleVehiculo.subestado_operativo ?? detalleVehiculo.estado_operativo.replace(/_/g, ' ')}
                    </Badge>
                  </div>
                </div>
              </CardContent>
