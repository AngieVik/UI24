...
const schemaFrenosNeum = z.object({
  matricula: z.string().min(1, 'Selecciona un vehículo'),
  km_actual: z.coerce.number().int().positive('Introduce el km actual'),
  resultado: z.enum(['conforme', 'no_conforme', 'advertencia'] as const, { message: 'Selecciona el resultado' }),
  notas:     z.string().optional(),
})
...
function FormMantenimiento({
  ...
  if (done) {
    return (
      <div className="flex flex-col items-center gap-2 py-6 text-center">
        ...
        <Button size="sm" variant="outline" onClick={() => setDone(false)}>Nuevo registro</Button>
      </div>
    )
  }
  ...
  if (tipo === 'aceite') {
    return (
      <form
        ...
        noValidate
      >
        <Controller control={aceiteForm.control} name="matricula" render={({ field, fieldState }) => (
          <Field data-invalid={fieldState.invalid}>
            ...
            <Select value={field.value} onValueChange={field.onChange} disabled={mut.isPending}>
              <SelectTrigger id="mant-ac-mat"><SelectValue placeholder="Seleccionar…" /></SelectTrigger>
              <SelectContent>{matriculas.map((m) => <SelectItem key={m} value={m}>{m}</SelectItem>)}</SelectContent>
            </Select>
            {fieldState.error && <FieldError errors={[fieldState.error]} />}
          </Field>
        )} />
        <Controller control={aceiteForm.control} name="km_actual" render={({ field, fieldState }) => (
          <Field data-invalid={fieldState.invalid}>
            <FieldLabel htmlFor="mant-ac-km">Km actual</FieldLabel>
            <Input {...field} id="mant-ac-km" type="number" min="0" disabled={mut.isPending} aria-invalid={fieldState.invalid} />
            {fieldState.error && <FieldError errors={[fieldState.error]} />}
          </Field>
        )} />
        <Controller control={aceiteForm.control} name="tipo_aceite" render={({ field, fieldState }) => (
          <Field data-invalid={fieldState.invalid}>
            <FieldLabel htmlFor="mant-ac-tipo">Tipo de aceite</FieldLabel>
            <Select value={field.value} onValueChange={field.onChange} disabled={mut.isPending}>
              <SelectTrigger id="mant-ac-tipo"><SelectValue /></SelectTrigger>
              <SelectContent>
                {['5W30', '5W40', '10W40', '15W40', '0W30'].map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}
              </SelectContent>
            </Select>
            {fieldState.error && <FieldError errors={[fieldState.error]} />}
          </Field>
        )} />
        <Controller control={aceiteForm.control} name="litros" render={({ field, fieldState }) => (
          <Field data-invalid={fieldState.invalid}>
            <FieldLabel htmlFor="mant-ac-lit">Litros añadidos</FieldLabel>
            <Input {...field} id="mant-ac-lit" type="number" step="0.1" min="0" disabled={mut.isPending} aria-invalid={fieldState.invalid} />
            {fieldState.error && <FieldError errors={[fieldState.error]} />}
          </Field>
        )} />
  ...
  return (
    <form
      ...
      noValidate
    >
      <Controller control={frenosForm.control} name="matricula" render={({ field, fieldState }) => (
        <Field data-invalid={fieldState.invalid}>
          <FieldLabel htmlFor={`mant-${tipo}-mat`}>Vehículo</FieldLabel>
          <Select value={field.value} onValueChange={field.onChange} disabled={mut.isPending}>
            <SelectTrigger id={`mant-${tipo}-mat`}><SelectValue placeholder="Seleccionar…" /></SelectTrigger>
            <SelectContent>{matriculas.map((m) => <SelectItem key={m} value={m}>{m}</SelectItem>)}</SelectContent>
          </Select>
          {fieldState.error && <FieldError errors={[fieldState.error]} />}
        </Field>
      )} />
      <Controller control={frenosForm.control} name="km_actual" render={({ field, fieldState }) => (
        <Field data-invalid={fieldState.invalid}>
          <FieldLabel htmlFor={`mant-${tipo}-km`}>Km actual</FieldLabel>
          <Input {...field} id={`mant-${tipo}-km`} type="number" min="0" disabled={mut.isPending} aria-invalid={fieldState.invalid} />
          {fieldState.error && <FieldError errors={[fieldState.error]} />}
        </Field>
      )} />
      <Controller control={frenosForm.control} name="resultado" render={({ field, fieldState }) => (
        <Field data-invalid={fieldState.invalid}>
          <FieldLabel htmlFor={`mant-${tipo}-res`}>Resultado revisión</FieldLabel>
          <Select value={field.value} onValueChange={field.onChange} disabled={mut.isPending}>
            <SelectTrigger id={`mant-${tipo}-res`}><SelectValue placeholder="Seleccionar…" /></SelectTrigger>
            <SelectContent>
...
export function MantenimientoFlotaScreen({ vista }: { vista?: 'aceite' | 'frenos' | 'neumaticos' | 'umbrales' }) {
    ...
    <div className="mx-auto flex w-full max-w-xl flex-col gap-3 p-3">
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="w-full">
          <TabsTrigger value="aceite"><Droplets className="size-3.5 mr-1" />Aceite</TabsTrigger>
          <TabsTrigger value="frenos"><Disc className="size-3.5 mr-1" />Frenos</TabsTrigger>
          <TabsTrigger value="neumaticos"><Disc3 className="size-3.5 mr-1" />Neumáticos</TabsTrigger>
          <TabsTrigger value="umbrales"><Sliders className="size-3.5 mr-1" />Umbrales</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="umbrales" className="mt-3">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="font-display text-base">Configuración de umbrales de alerta</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="font-body text-sm text-muted-foreground">
                Configura los umbrales de km para alertas de mantenimiento preventivo (aceite, frenos, neumáticos).
                Solo disponible para responsables de flota.
              </p>
