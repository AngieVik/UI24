...
const SECCION_OPTIONS: AnuncioItem['seccion'][] = ['normativas', 'protocolos', 'avisos_corporativos']
...
function GestionTablon() {
  ...
  return (
    <div className="space-y-3">
      <Card>
        ...
        <CardContent className="space-y-3">
          <Field>
            ...
            <Select value={seccion} onValueChange={(v) => setSeccion(v as AnuncioItem['seccion'])} disabled={submitting}>
              <SelectTrigger id="tab-seccion"><SelectValue /></SelectTrigger>
              <SelectContent>
                {SECCION_OPTIONS.map((s) => <SelectItem key={s} value={s}>{SECCION_LABEL[s]}</SelectItem>)}
              </SelectContent>
            </Select>
          </Field>
          <Field>
            <FieldLabel htmlFor="tab-titulo">Título</FieldLabel>
            <Input id="tab-titulo" value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Título del anuncio" disabled={submitting} />
          </Field>
          <Field>
            <FieldLabel htmlFor="tab-contenido">Contenido</FieldLabel>
            <Textarea id="tab-contenido" value={contenido} onChange={(e) => setContenido(e.target.value)} rows={4} placeholder="Redacta el anuncio…" disabled={submitting} />
          </Field>
      ...
      {loading ? <Skeleton className="h-40 w-full" /> : anuncios.length === 0 ? (
              ...
              <CardContent className="flex items-start justify-between gap-3 py-3">
                <div className="space-y-0.5">
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge variant={SECCION_VARIANT[a.seccion]} className="text-xs">{SECCION_LABEL[a.seccion]}</Badge>
                    <span className="font-medium text-sm">{a.titulo}</span>
                  </div>
                  <p className="text-xs text-muted-foreground line-clamp-2">{a.contenido}</p>
                  <p className="text-xs text-muted-foreground">{fmtDateTime(a.timestamp_publicacion)}</p>
                </div>
                <Button size="sm" variant="ghost" onClick={() => handleArchivar(a.id_anuncio)} aria-label={`Archivar ${a.titulo}`}>
      ...
      <CardContent className="space-y-3">
        ...
        {loading ? <Skeleton className="h-16 w-full" /> : (
          <>
            ...
            <Button size="sm" className="w-full" onClick={handleGuardar} disabled={saving || !texto.trim()}>
    ...
    <div className="mx-auto flex w-full max-w-xl flex-col gap-3 p-3">
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="w-full">
          <TabsTrigger value="tablon"><Newspaper className="size-3.5 mr-1" />Tablón</TabsTrigger>
          <TabsTrigger value="marquesina"><RadioTower className="size-3.5 mr-1" />Marquesina</TabsTrigger>
        </TabsList>
        <TabsContent value="tablon" className="mt-3"><GestionTablon /></TabsContent>
        <TabsContent value="marquesina" className="mt-3"><GestionMarquesina /></TabsContent>
      </Tabs>
    </div>
  )
}
