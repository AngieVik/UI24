...
import { CalendarDays, ChevronLeft, ChevronRight, Plus, RefreshCw, Settings2, X } from 'lucide-react'
...
const TIPO_OPTIONS = [
  'SVB emergencia', 'SVA emergencia', 'Traslado programado',
  'Traslado urgente', 'Guardia base', 'Apoyo DRP',
]
...
function fmtDate(iso: string): string {
  return new Date(iso).toLocaleDateString('es-ES', { weekday: 'short', day: '2-digit', month: '2-digit' })
}
...
export function ServiciosScreen() {
  ...
  async function handleAgregar() {
    if (!idNombre || !tipoServicio || !fecha) { setError('Completa empleado, tipo y fecha.'); return }
    setSubmitting(true)
    setError(null)
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      setShowForm(false)
      setMatricula(''); setIdNombre(''); setTipoServicio('')
    } catch (e) {
    ...
    <div className="mx-auto flex w-full max-w-screen-xl flex-col gap-3 p-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          {query.data && (
            <Badge variant="secondary">{query.data.filter((s) => s.estado !== 'Cancelado').length} servicios</Badge>
          )}
        </div>
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={() => { setShowForm(!showForm); setError(null) }}>
            {showForm ? 'Cancelar' : <><Plus className="size-4" />Nuevo servicio</>}
          </Button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {weekOffset !== 0 && (
          <Button size="sm" variant="ghost" onClick={() => setWeekOffset(0)}>Esta semana</Button>
        )}
      </div>
          ...
          <CardContent className="space-y-3">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <Field>
                ...
                <Input id="srv-fecha" type="date" value={fecha} onChange={(e) => setFecha(e.target.value)} disabled={submitting} />
              </Field>
              <Field>
                <FieldLabel htmlFor="srv-turno">Turno</FieldLabel>
                <Select value={turno} onValueChange={setTurno} disabled={submitting}>
                  <SelectTrigger id="srv-turno"><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <Field>
                ...
                <Select value={idNombre} onValueChange={setIdNombre} disabled={submitting || empQuery.isLoading}>
                  <SelectTrigger id="srv-empleado"><SelectValue placeholder="Seleccionar…" /></SelectTrigger>
                  <SelectContent>
                    {(empQuery.data ?? []).map((e) => (
              ...
              <Field>
                ...
                <Select value={tipoServicio} onValueChange={setTipoServicio} disabled={submitting}>
                  <SelectTrigger id="srv-tipo"><SelectValue placeholder="Seleccionar…" /></SelectTrigger>
                  <SelectContent>
              ...
              <Field>
                <FieldLabel htmlFor="srv-matricula">Matrícula <span className="font-light text-muted-foreground">— opcional</span></FieldLabel>
                <Input id="srv-matricula" value={matricula} onChange={(e) => setMatricula(e.target.value)} placeholder="Ej. 1234 ABC" disabled={submitting} />
              </Field>
            </div>
      ...
      {query.isLoading ? (
        ...
        <Card>
          <CardContent className="py-8 text-center">
            <p className="text-sm text-muted-foreground">No hay servicios planificados para esta semana.</p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-4">
          {Object.entries(byFecha).map(([fecha, servicios]) => (
            <div key={fecha}>
              <p className="mb-2 text-xs font-bold uppercase text-muted-foreground">{fmtDate(fecha)}</p>
              <div className="space-y-2">
                {servicios.map((s) => (
                  <Card key={s.id}>
                    <CardContent className="flex items-center justify-between gap-3 py-3">
                      <div className="flex flex-wrap items-center gap-2">
                        <Badge variant={TURNO_VARIANT[s.turno] ?? 'secondary'} className="text-xs w-16 justify-center">
                        ...
                        {s.matricula && (
                          <Badge variant="outline" className="text-xs font-mono">{s.matricula}</Badge>
                        )}
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge variant={ESTADO_VARIANT[s.estado] ?? 'secondary'} className="text-xs">{s.estado}</Badge>
                        {s.estado !== 'Cancelado' && (
                          <Button
