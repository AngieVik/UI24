...
const NIVEL_CONFIG: Record<
  AvisoItem['nivel'],
  { label: string; variant: 'destructive' | 'warn' | 'info' }
> = {
...
function AvisoRow({ aviso, ejecutorId, onMarcarLeido }: AvisoRowProps) {
  ...
  return (
    <Card aria-label={`Aviso: ${aviso.tipo_aviso}`} className={yaLeido ? 'opacity-60' : ''}>
      <CardContent className="flex items-start justify-between gap-3 py-3">
        <div className="flex min-w-0 flex-col gap-1">
          ...
          <p className="text-xs text-muted-foreground">
            Emitido por:{' '}
            <span className="font-medium text-foreground">{aviso.id_nombre_emisor}</span>
          </p>
        </div>
        <Button
          size="sm"
  ...
  if (!idActivacion) {
    return (
      <div className="mx-auto flex max-w-xl flex-col items-center gap-3 px-6 py-16 text-center">
        ...
        <h2 className="font-display text-lg font-bold leading-tight">Doc-11 — Avisos urgentes</h2>
